...
sub get_multipart {
  ...
  Multipart:
	while (!&_end_of_multipart()) {
		...
		unless (defined ($field)) { $field = 'undefined'; }
		...
		if ($fname) {	# 添付ファイル
			...
			$upload->{filesize} = -s $upload->{tmpfile};
...
sub _new_upload_file {
	...
	# シフトJISと仮定して処理している。
	my $sjis_c1 = '\x81-\x9f\xe0-\xfc';
	($basename) = $filename =~ /^(?:[^$sjis_c1]*(?:[$sjis_c1].[^$sjis_c1]*)*[:\\\/])?([^:\/]+)/o;
