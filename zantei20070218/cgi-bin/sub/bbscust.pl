...
# 新しい項目を追加
	...
	my ( $opt5 , $opt6 , $opt1 , $opt2 , $opt3 , $opt4 , $opt7 , $opt8 , $opt9 , $opt10 , $opt11 , $opt12 , $opt13 ,$opt14 , $opt15 );
	...
	if ( $FORM{'ztree'} ) {
		$opt9 = "&tree=$FORM{'ztree'}";
	}

	if ( $FORM{'zwindow'} ) {
		$opt10 = "&zwindow=$FORM{'zwindow'}";
	}

	if ( $FORM{'limitcfg'} ) {
		$opt11 = "&limitcfg=$FORM{'limitcfg'}";
	}

	if ( $FORM{'linelimit'} ) {
		$opt12 = "&linelimit=$FORM{'linelimit'}";
	}

	if ( $FORM{'collimit'} ) {
		$opt13 = "&collimit=$FORM{'collimit'}";
	}

	if ( $FORM{'lineswitch'} ) {
		$opt14 = "&lineswitch=$FORM{'lineswitch'}";
	}

	if ( $FORM{'colswitch'} ) {
		$opt15 = "&colswitch=$FORM{'colswitch'}";
	}


$optcust = "$opt5$opt6$opt1$opt2$opt3$opt4$opt7$opt8$opt9$opt10$opt11$opt12$opt13$opt14$opt15";
